...
const UpdatePost = () => {
  ...
  useEffect(() => {
    const getPostInfo = async () => {
      setDataLoading(true);
      const res = await fetch(`${API_BASE}/api/posts/${params.id}`);
  ...
  const handleFormSubmit = async (data) => {
    try {
      ...
      const res = await  fetch(`${API_BASE}/api/posts/update/${params.id}`, {
        method: "POST",
        headers: {
                  ...
                  <div>
                    ...
                    <div className="image_upload_container md:p-5 md:border-2 bg-transparent border-dashed rounded-sm md:flex items-center justify-center gap-2">
                      ...
                      <button
                        ...
                        } md:h-[3rem] md:mt-0 duration-500 hover:shadow-lg disabled:border-yellow-500 disabled:text-yellow-500`}
                      >

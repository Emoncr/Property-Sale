...
const Chat = ({ conversationInfo }) => {
    ...
    (async () => {
      try {
        ...
        const res = await fetch(
          `${API_BASE}/api/message?sender=${trackConversation.sender}&receiver=${trackConversation.receiver}`
        );
  ...
  const handleSendMsg = async (e) => {
    ...
    sendMessageTOSocket();
    try {
      const sendMsgToDB = await  fetch(`${API_BASE}/api/message/create`, {
        method: "POST",
  ...
  const handleConversationDelete = async () => {
    try {
      const deleteChat = await fetch(
        `${API_BASE}/api/conversation/delete/${_id}`,
        {
          method: "DELETE",
        }
      );
    ...
    <>
      {messageLoading ? (
        <div className="w-full">
          <p className="text-center mt-20 font-bold font-heading text-sm text-black">
            Messages Loading...
          </p>
        </div>
      ) : (
                          ...
                          >
                            <div className="User_chat flex items-center gap-2 mt-2 ">
                              <img
                                ...
                                alt="chat partner image"
                              />
                              <p className="text-lg font-normal bg-blue-500 px-2 text-white py-1 rounded-md">
                                {msg.message}
                              </p>
                            </div>
                          </div>
